...
To publish a standalone release and update NitroBolt Desktop, run the `Release standalone packager` GitHub Actions workflow manually and provide a semantic version. The workflow updates `package.json`, builds and releases the standalone file, and requires an `AUTOMATION` organization secret with permission to dispatch workflows and push to the NitroBolt Desktop repository.
